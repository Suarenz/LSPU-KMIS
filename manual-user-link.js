...
async function manualUserLink() {
  console.log('Starting manual user linking...');
  ...
  try {
    ...
    for (const userData of defaultUsers) {
      ...
      if (existingUser) {
        // Update existing user with the Supabase auth ID if not already set
        ...
        console.log(`  - User ${userData.email} does not exist in database, will create...`);
    ...
    console.log('\nManual user linking completed!');
  } catch (error) {
    console.error('Error during manual user linking:', error);
  } finally {
...
// Run the function
manualUserLink().catch(console.error);