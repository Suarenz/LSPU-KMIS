...
"[project]/lib/utils/kpi-type-logic.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function getKpiTypeCategory(kpiType) {
...
function getGapInterpretation(category) {
    switch(category){
        case 'VOLUME':
            return {
                category: 'VOLUME',
        ...
        default:
            return {
                ...
                rootCauseFocus: [
                    'General performance review required'
                ],
...
function generateTypeSpecificLogicInstruction(kpiType) {
    const category = getKpiTypeCategory(kpiType);
    const interpretation = getGapInterpretation(category);
    const rootCauseList = interpretation.rootCauseFocus.map((cause, i)=>`   ${i + 1}. ${cause}`).join('\n');
...
function generateTypeAwareRecommendation(kpiType, kpiName, actualValue, targetValue, achievementPercent) {
    const category = getKpiTypeCategory(kpiType);
    const interpretation = getGapInterpretation(category);
...
function validatePrescriptiveAnalysis(analysis, kpiType) {
    const category = getKpiTypeCategory(kpiType);
    const warnings = [];
    ...
    if (category === 'EFFICIENCY') {
        ...
        for (const pattern of volumePatterns){
            if (pattern.test(analysis)) {
                warnings.push(`Anti-pattern detected: Suggesting data collection/reporting fixes for an EFFICIENCY metric. ` + `For rate/percentage KPIs, focus on quality improvement (curriculum, training, standards), not data volume.`);
                break;
            }
        }
    }
    if (category === 'MILESTONE') {
        ...
        for (const pattern of incrementalPatterns){
            if (pattern.test(analysis)) {
                warnings.push(`Anti-pattern detected: Suggesting incremental improvements for a MILESTONE metric. ` + `For milestone/status KPIs, focus on unblocking specific delays through intervention.`);
                break;
            }
        }
    }
...
function getKpiTypeDescription(kpiType) {
    const category = getKpiTypeCategory(kpiType);
    const interpretation = getGapInterpretation(category);
    return `${category} (${interpretation.gapType}) - ${interpretation.actionArchetype}`;
}
function buildTypeAwarePromptContext(kpiType, kpiName, kraName, targetValue, actualValue, gap) {
    const category = getKpiTypeCategory(kpiType);
...
var __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$utils$2f$kpi$2d$type$2d$logic$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/lib/utils/kpi-type-logic.ts [app-route] (ecmascript)");
 ...
 */ async function generatePrescriptiveAnalysis(activities, kraId, kraTitle, reportYear) {
    ...
    const kpiSummaries = Array.from(byInitiative.entries()).map(([initiativeId, acts])=>{
        ...
        const kpiTypeCategory = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$utils$2f$kpi$2d$type$2d$logic$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getKpiTypeCategory"])(rawKpiType);
        const gapInterpretation = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$utils$2f$kpi$2d$type$2d$logic$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getGapInterpretation"])(kpiTypeCategory);
    ...
    const uniqueKpiTypes = [
        ...new Set(kpiSummaries.map((k)=>k.kpiTypeCategory))
    ];
    const typeSpecificRules = uniqueKpiTypes.map((category)=>{
        const interpretation = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$utils$2f$kpi$2d$type$2d$logic$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getGapInterpretation"])(category);
...
`;
    try {
        ...
        let prescriptiveItems = shouldAllowSustain ? rawItems : rawItems.filter((x)=>!/\bsustain\b|high\s*perform(er|ers)|preserv(e|ing)\s+strong/i.test(`${x.title} ${x.issue} ${x.action}`));
        ...
        if (hasEfficiencyMetrics) {
            prescriptiveItems = prescriptiveItems.map((item)=>{
                ...
                const validation = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$utils$2f$kpi$2d$type$2d$logic$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["validatePrescriptiveAnalysis"])(combinedText, 'percentage');
                if (!validation.isValid) {
                    ...
                    return {
                        ...item,
                        action: item.action.replace(/reporting\s+(bottleneck|backlog|delay|lag)/gi, 'quality improvement initiative').replace(/batch\s+(collection|processing|data)/gi, 'targeted intervention').replace(/collect\s+more\s+(data|lists|reports)/gi, 'improve program outcomes').replace(/scale\s+up\s+(data|collection|reporting)/gi, 'enhance quality standards').replace(/data\s+collection\s+(delay|issue|problem)/gi, 'outcome quality concern')
                    };
                }
                return item;
            });
        ...
        const bottleneckInterpretation = bottleneck?.gapInterpretation || (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$utils$2f$kpi$2d$type$2d$logic$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getGapInterpretation"])('UNKNOWN');
        ...
        if (bottleneck?.initiativeId) {
            ...
            const gapDescription = bottleneckTypeCategory === 'EFFICIENCY' ? `This indicates a ${bottleneckInterpretation.gapType.toLowerCase()} requiring quality-focused interventions.` : bottleneckTypeCategory === 'MILESTONE' ? `This indicates a ${bottleneckInterpretation.gapType.toLowerCase()} requiring administrative intervention.` : `This represents a ${bottleneckInterpretation.gapType.toLowerCase()}.`;
            documentInsightParts.push(`The primary bottleneck is ${bottleneck.initiativeId} at ${Number(bottleneck.achievementPercent || 0).toFixed(1)}% (reported ${bottleneckReported} vs target ${bottleneckTarget}). ${gapDescription}`);
        }
        ...
        return {
            ...
            gaps: bottleneck?.initiativeId ? `Largest gap is ${bottleneck.initiativeId}: ${Number(bottleneck.achievementPercent || 0).toFixed(1)}% achieved (reported ${bottleneckReported} vs target ${bottleneckTarget}). Gap type: ${bottleneckInterpretation.gapType}.` : `${missedCount} KPI(s) are below target.`,
...
class QPROAnalysisService {
   ...
   */ validateAndFixActivityKRAMatches(activities, strategicPlan) {
        const correctedActivities = activities.map((activity)=>{
            ...
            const findBestKpi = (act, targetKraId)=>{
                const strategicPlanKras = strategicPlan && strategicPlan.kras || [];
                const normalizedTargetKraId = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$utils$2f$qpro$2d$aggregation$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["normalizeKraId"])(targetKraId);
                const targetKra = strategicPlanKras.find((kra)=>(0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$utils$2f$qpro$2d$aggregation$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["normalizeKraId"])(kra.kra_id) === normalizedTargetKraId);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__4e21c9b9._.js.map